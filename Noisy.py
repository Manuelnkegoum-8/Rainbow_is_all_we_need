...
class NoisyLayer(nn.Module):
    ...
    def __init__(self,in_features,out_features):
        ...
        self.register_buffer("noise_b", torch.Tensor(out_features))
        self.register_buffer(
            "noise_w", torch.Tensor(out_features, in_features)
        )
    ...
    def f_noise(self,x):
        return torch.sign(x).mul(torch.sqrt(torch.abs(x)))
    ...
    def forward(self,x):
        weights = self.weight_mu + self.weight_std*self.noise_w
        bias = self.bias_mu + self.bias_std*self.noise_b
