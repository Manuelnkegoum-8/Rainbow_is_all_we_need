...
parser.add_argument('--memory_capacity', type=int, default=int(3e4), metavar='CAPACITY', help='Experience replay memory capacity')
...
def train(agent,optimizer,env,buffer,batch_size,beta,n_steps,gamma,device):
    ...
    for steps_done in trange(1,Max_steps_done+1,ncols=80):
        ...
        action = agent.select_action(state_) # choose an action
        ...
        if steps_done > min_samples:
            ...
            if steps_done%train_freq==0:
                optimize(agent,buffer,optimizer,batch_size,n_steps,gamma,beta_,device)
...
if __name__=='__main__':
    ...
    device = args.device
    state, _ = env.reset()
    agent = RainbowAgent(env,hidden,gamma,TAU,vmin,vmax,atom_size,n_steps,None,device)
